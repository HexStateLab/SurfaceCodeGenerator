    ...
    for(int i=1;i<argc;i++) {
        if(!strcmp(argv[i],"--bench")) bench=1;
            ...
            for(int pass=0;pass<15;pass++){
                ...
                if(pass>0){
                    memset(coarse_votes[0],0,coarse_sz[0]*sizeof(int)*4); // zero all
                    for(rnd=0;rnd<rounds;rnd++){
                        uint8_t *rdat=all_rounds+rnd*n;
                        for(int q=0;q<n;q++) rdat[q]^=guess_syn[q]; // remove explained syndrome
                        for(int f=0;f<4;f++){
                            if(!coarse_votes[f]) continue;
                            int hrc=hrc_arr[f], hsc=hsc_arr[f], *v=coarse_votes[f];
                            for(int a=0;a<hrc;a++)for(int b=0;b<hsc;b++){
                        ...
                        for(int q=0;q<n;q++) rdat[q]^=guess_syn[q]; // restore
                    }
