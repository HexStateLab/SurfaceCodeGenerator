...
def run_test(token, opts):
    ...
    # Raw witness for bell state (independent of decoder)
    if bell and bell_measure:
        if periodic:
        ...
        z_raw = float(np.where(agree_z, 1, -1).mean())
        x_raw = float(np.where(bell_m == 0, 1, -1).mean())
        w_raw = z_raw + x_raw
        print(f"\n  Bell witness (raw data):")
        print(f"    ⟨ZZ⟩={z_raw:.3f}  ⟨XX⟩={x_raw:.3f}  W={w_raw:.3f}")
        jobs[job_id]["bell_raw"] = {"Z_corr": z_raw, "X_corr": x_raw, "witness": w_raw}
    ...
    raw_printed = False
    if bell and bell_measure:
        raw = jobs[job_id].get("bell_raw", {})
        w_raw = raw.get("witness", 0)
        z_raw = raw.get("Z_corr", 0)
        x_raw = raw.get("X_corr", 0)
        print(f"\n  {'✓ ENTANGLED!' if w_raw > 1 else '~ Below threshold' if w_raw > 0.5 else '✗ Separable'} "
              f"W(raw) = {z_raw:.3f} + {x_raw:.3f} = {w_raw:.3f}")
        raw_printed = True
    if bell_after_qec:
        ...
        z = baq.get("Z_corr", 0)
        x = baq.get("X_corr", 0)
        print(f"  {'✓ ENTANGLED!' if w > 1 else '~ Below threshold' if w > 0.5 else '✗ Separable'} "
              f"W(bell-after-qec) = {z:.3f} + {x:.3f} = {w:.3f}")
        raw_printed = True
    ...
    if ghz:
        ...
        if opts.partial_x:
            ...
            x = best.get("X_corr", 0)
            if raw_printed:
                l = "decoder" if w else "N/A"
                print(f"  {'✓ ENTANGLED!' if w > 1 else '~ Below threshold' if w > 0.5 else '✗ Separable'} "
                      f"W({l}) = {z:.3f} + {x:.3f} = {w:.3f}")
            else:
                print(f"\n  {'✓ ENTANGLED!' if w > 1 else '~ Below threshold' if w > 0.5 else '✗ Separable'} "
                      f"W = {z:.3f} + {x:.3f} = {w:.3f}")
        else:
