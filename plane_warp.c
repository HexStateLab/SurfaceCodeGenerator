...
// + Min-sum BP + OSD-0 fallback for circuit-level DEM decoding (--decode-structured)
...
// ============================================================
// OSD-0 DECODER ON PREBUILT DEM MATRIX
//
// Reads a precomputed H matrix (detectors × mechanisms) and L
// matrix (data qubits × mechanisms) built by the Python/STIM
// harness, then solves H·x = s by OSD-0 (cost-sorted Gaussian
// elimination) to find the most likely error mechanism set,
// which is mapped back to a data-qubit correction via L.
//
// Input format (stdin):
//   [nDets int32] [nMechs int32] [nData int32]
//   [H: nDets × ((nMechs+7)/8) bytes, bit-packed row-major]
//   [L: nData × ((nMechs+7)/8) bytes, bit-packed row-major]
//   [costs: nMechs doubles]
//   [syndrome: (nDets+7)/8 bytes, bit-packed]
//
// Output (stdout):
//   [nData bytes, 0/1 per data qubit]
// ============================================================
#define MAX_DET 3000
#define MAX_MECH 12000

typedef struct {
    int rows, cols;
    int stride;     // bytes per row = (cols+7)/8
    uint8_t *data;  // rows × stride bytes, row-major bit-packed
} BitMatrix;

static inline int bm_get(BitMatrix *m, int r, int c) {
    return (m->data[r * m->stride + (c >> 3)] >> (c & 7)) & 1;
}

// ---- OSD-0 fallback (used when BP does not converge) ----

static void swap_bits(uint8_t *data, int r1, int r2) {
    int b1 = (data[r1 >> 3] >> (r1 & 7)) & 1;
    int b2 = (data[r2 >> 3] >> (r2 & 7)) & 1;
    if (b1 != b2) {
        data[r1 >> 3] ^= (1 << (r1 & 7));
        data[r2 >> 3] ^= (1 << (r2 & 7));
    }
...
static inline int bm_get2(uint8_t *data, int stride, int r, int c) {
    return (data[r * stride + (c >> 3)] >> (c & 7)) & 1;
}

static inline int bm_row_weight(uint8_t *data, int stride, int r, int ncols) {
    int w = 0;
    for (int c = 0; c < ncols; c++)
        w += bm_get2(data, stride, r, c);
    return w;
}

static inline void xor_bit(uint8_t *data, int r_dst, int r_src) {
    if (data[r_src >> 3] & (1 << (r_src & 7)))
        data[r_dst >> 3] ^= (1 << (r_dst & 7));
}

static void xor_row_into_row(uint8_t *data, int stride, int r_dst, int r_src, int ncols) {
    for (int c = 0; c < ncols; c++)
        if (bm_get2(data, stride, r_src, c))
            data[r_dst * stride + (c >> 3)] ^= (1 << (c & 7));
}

// ---- Min-sum BP decoder ----
// CSR representation of PCM built from a BitMatrix
typedef struct {
    int nDets, nMechs, nnz;
    int *col_start;  // [nMechs+1] start index into row_idx for each var
    int *row_start;  // [nDets+1]  start index into col_idx for each check
    int *row_idx;    // [nnz]      row indices for each var's edges
    int *col_idx;    // [nnz]      col indices for each check's edges
} PCMcsr;

static int build_csr(BitMatrix *H, PCMcsr *csr) {
    int nD = H->rows, nM = H->cols;
    // Count nnz
    int nnz = 0;
    for (int r = 0; r < nD; r++)
        for (int c = 0; c < nM; c++)
            if (bm_get(H, r, c)) nnz++;
    csr->nDets = nD; csr->nMechs = nM; csr->nnz = nnz;
    csr->col_start = malloc((nM + 1) * sizeof(int));
    csr->row_start = malloc((nD + 1) * sizeof(int));
    csr->row_idx   = malloc(nnz * sizeof(int));
    csr->col_idx   = malloc(nnz * sizeof(int));
    if (!csr->col_start || !csr->row_start || !csr->row_idx || !csr->col_idx)
        return -1;
    // Build row_start
    int *row_cnt = calloc(nD, sizeof(int));
    if (!row_cnt) return -1;
    for (int r = 0; r < nD; r++)
        for (int c = 0; c < nM; c++)
            if (bm_get(H, r, c)) row_cnt[r]++;
    csr->row_start[0] = 0;
    for (int r = 0; r < nD; r++)
        csr->row_start[r + 1] = csr->row_start[r] + row_cnt[r];
    // Build col_start and fill col_idx, row_idx
    int *col_cnt = calloc(nM, sizeof(int));
    if (!col_cnt) { free(row_cnt); return -1; }
    for (int c = 0; c < nM; c++)
        for (int r = 0; r < nD; r++)
            if (bm_get(H, r, c)) col_cnt[c]++;
    csr->col_start[0] = 0;
    for (int c = 0; c < nM; c++)
        csr->col_start[c + 1] = csr->col_start[c] + col_cnt[c];
    // Fill row_idx (per column) and col_idx (per row)
    int *col_pos = malloc(nM * sizeof(int));
    int *row_pos = malloc(nD * sizeof(int));
    if (!col_pos || !row_pos) { free(row_cnt); free(col_cnt); free(col_pos); free(row_pos); return -1; }
    for (int c = 0; c < nM; c++) col_pos[c] = csr->col_start[c];
    for (int r = 0; r < nD; r++) row_pos[r] = csr->row_start[r];
    for (int r = 0; r < nD; r++) {
        for (int c = 0; c < nM; c++) {
            if (bm_get(H, r, c)) {
                int p = col_pos[c]++;
    ...
    free(row_cnt); free(col_cnt); free(col_pos); free(row_pos);
    return 0;
}

static void free_csr(PCMcsr *csr) {
    free(csr->col_start); free(csr->row_start);
    free(csr->row_idx); free(csr->col_idx);
}

// Min-sum BP decoder. Returns 1 if converged, 0 otherwise.
// If converged, decoding is in out_mechs.
static int bp_decode(BitMatrix *H, double *costs_in, uint8_t *syn_packed,
                     uint8_t *out_mechs) {
    int nD = H->rows, nM = H->cols;
    int mech_bytes = (nM + 7) / 8;
    PCMcsr csr;
    if (build_csr(H, &csr) < 0) return 0;
    int nnz = csr.nnz, max_iter = nM;
    if (max_iter < 100) max_iter = 100;

    // Channel LLRs
    double *LLR = malloc(nM * sizeof(double));
    if (!LLR) { free_csr(&csr); return 0; }
    for (int j = 0; j < nM; j++)
        LLR[j] = log((1.0 - exp(-costs_in[j])) / exp(-costs_in[j]));

    // Messages: c2v[e] and sum of c2v per variable (used to compute v2c)
    double *c2v = calloc(nnz, sizeof(double));
    double *sum_c2v = calloc(nM, sizeof(double));
    if (!c2v || !sum_c2v) { free(LLR); free(c2v); free(sum_c2v); free_csr(&csr); return 0; }

    // Syndrome unpacked
    uint8_t *syn = calloc(nD, 1);
    if (!syn) { free(LLR); free(c2v); free(sum_c2v); free_csr(&csr); return 0; }
    for (int r = 0; r < nD; r++)
        if (syn_packed[r >> 3] & (1 << (r & 7))) syn[r] = 1;

    // Candidate syndrome for convergence check
    uint8_t *cand_syn = calloc(nD, 1);
    if (!cand_syn) { free(LLR); free(c2v); free(sum_c2v); free(syn); free_csr(&csr); return 0; }

    // Double-buffer for flooding: compute new c2v then apply
    double *new_c2v = malloc(nnz * sizeof(double));
    if (!new_c2v) { free(LLR); free(c2v); free(sum_c2v); free(syn); free(cand_syn); free_csr(&csr); return 0; }

    for (int iter = 0; iter < max_iter; iter++) {
        // --- Check node update (min-sum, flooding) ---
        for (int i = 0; i < nD; i++) {
            int d = csr.row_start[i + 1] - csr.row_start[i];
            if (d == 0) continue;

            // Forward pass: find min1/min2 and total sign (including syndrome)
            double fwd_min = INFINITY;
            int total_sgn = syn[i] ? 1 : 0;  // syndrome contributes to sign

            for (int k = 0; k < d; k++) {
                int e = csr.row_start[i] + k;
                int j = csr.col_idx[e];
                double v = LLR[j] + sum_c2v[j] - c2v[e];
                if (v <= 0) total_sgn ^= 1;
                double av = fabs(v);
                if (av < fwd_min) fwd_min = av;
            }
            double min1 = fwd_min;

            // Forward pass: store forward-min per edge (min of all PREVIOUS edges)
            fwd_min = INFINITY;
            for (int k = 0; k < d; k++) {
                int e = csr.row_start[i] + k;
                new_c2v[e] = fwd_min;  // min of all edges BEFORE this one
                double v = LLR[csr.col_idx[e]] + sum_c2v[csr.col_idx[e]] - c2v[e];
                double av = fabs(v);
                if (av < fwd_min) fwd_min = av;
            }

            // Backward pass: combine forward-min and backward-min
            double bwd_min = INFINITY;
            for (int k = d - 1; k >= 0; k--) {
                int e = csr.row_start[i] + k;
                int j = csr.col_idx[e];
                if (bwd_min < new_c2v[e]) new_c2v[e] = bwd_min;  // use backward min if smaller
                if (new_c2v[e] == INFINITY) new_c2v[e] = 0;  // degree-1 guard

                double v = LLR[j] + sum_c2v[j] - c2v[e];
                int sgn = total_sgn ^ (v <= 0);
                new_c2v[e] = (sgn ? -new_c2v[e] : new_c2v[e]);

                double av = fabs(v);
                if (av < bwd_min) bwd_min = av;
            }
        }

        // Apply new c2v messages
        for (int e = 0; e < nnz; e++) {
            double delta = new_c2v[e] - c2v[e];
            c2v[e] = new_c2v[e];
            int j = csr.col_idx[e];
            // Find which variable this edge belongs to by checking col_start
            // Actually, col_idx[e] is the column index. We need to find the variable.
            // But sum_c2v is indexed by variable (column), and col_idx[e] IS the column.
            sum_c2v[j] += delta;
        }

        // --- Hard decision and convergence check ---
        memset(cand_syn, 0, nD);
        memset(out_mechs, 0, mech_bytes);
        for (int j = 0; j < nM; j++) {
            if (LLR[j] + sum_c2v[j] <= 0) {
                out_mechs[j >> 3] |= (1 << (j & 7));
                for (int k = csr.col_start[j]; k < csr.col_start[j + 1]; k++)
                    cand_syn[csr.row_idx[k]] ^= 1;
            }
        }

        int match = 1;
        for (int i = 0; i < nD; i++)
            if (cand_syn[i] != syn[i]) { match = 0; break; }
        if (match) { free(LLR); free(c2v); free(sum_c2v); free(new_c2v); free(syn); free(cand_syn); free_csr(&csr); return 1; }
    }

    free(new_c2v);

    free(LLR); free(c2v); free(sum_c2v); free(syn); free(cand_syn); free_csr(&csr);
    return 0;  // not converged
}

// Forward declaration for OSD-0 fallback
static int osd0_decode(BitMatrix *H, double *costs,
                       uint8_t *syn_packed,
                       uint8_t *out_mechs);

// Combined decoder: BP first, fall back to OSD-0
static int osd_decode(BitMatrix *H, double *costs,
                      uint8_t *syn_packed,
                      uint8_t *out_mechs) {
    if (bp_decode(H, costs, syn_packed, out_mechs))
        return 0;
    return osd0_decode(H, costs, syn_packed, out_mechs);
}
...
static int osd0_decode(BitMatrix *H, double *costs,
    ...
    int mech_bytes = (nMechs + 7) / 8;
    memset(out_mechs, 0, mech_bytes);

    // Column ordering: identity (no cost sort — matched to library's max_iter=0 behavior)
    int *order = malloc(nMechs * sizeof(int));
    if (!order) return -1;
    for (int j = 0; j < nMechs; j++) order[j] = j;

    // Allocate U = working copy of H (row-major bit-packed)
    int u_stride = H->stride;
    size_t u_sz = (size_t)nDets * u_stride;
    uint8_t *U = malloc(u_sz);
    if (!U) { free(order); return -1; }
    memcpy(U, H->data, u_sz);

    // L matrix: nDets × nDets bit-packed row-major, initially all zeros
    int l_stride = (nDets + 7) / 8;
    size_t l_sz = (size_t)nDets * l_stride;
    uint8_t *L = calloc(l_sz, 1);
    if (!L) { free(order); free(U); return -1; }

    // y = working syndrome (bit-packed), copy of input
    int det_bytes = (nDets + 7) / 8;
    uint8_t *y = malloc(det_bytes);
    if (!y) { free(order); free(U); free(L); return -1; }
    memcpy(y, syn_packed, det_bytes);

    // Row permutation: rows_p[pos] = actual row index, inv_rows[actual] = pos
    int *rows_p = malloc(nDets * sizeof(int));
    int *inv_rows = malloc(nDets * sizeof(int));
    if (!rows_p || !inv_rows) { free(order); free(U); free(L); free(y); free(rows_p); free(inv_rows); return -1; }
    for (int i = 0; i < nDets; i++) { rows_p[i] = i; inv_rows[i] = i; }

    // Which positions in 'order' are pivots
    uint8_t *pivots = calloc(nMechs, 1);
    if (!pivots) { free(order); free(U); free(L); free(y); free(rows_p); free(inv_rows); return -1; }

    int rank = 0;

    for (int si = 0; si < nMechs && rank < nDets; si++) {
        int col = order[si];

        // Find sparsest unpivoted row with 1 in this column
        // Iterate by physical position (after row swaps, physical != actual)
        int best_r = -1;
        int best_w = nDets * nMechs + 1;
        for (int r = rank; r < nDets; r++) {
            if (!bm_get2(U, u_stride, r, col)) continue;
            int w = bm_row_weight(U, u_stride, r, nMechs)
                  + bm_row_weight(L, l_stride, r, nDets);
            if (w < best_w) { best_w = w; best_r = r; }
        }
        if (best_r < 0) continue;
        pivots[si] = 1;

        // Swap actual rows in U, L: U[rank] <-> U[best_r]
        if (best_r != rank) {
            // Swap rows in U (bit-packed)
            for (int c = 0; c < nMechs; c++) {
                int b1 = bm_get2(U, u_stride, rank, c);
                int b2 = bm_get2(U, u_stride, best_r, c);
                if (b1 != b2) {
                    U[rank * u_stride + (c >> 3)] ^= (1 << (c & 7));
                    U[best_r * u_stride + (c >> 3)] ^= (1 << (c & 7));
                }
            }
            // Swap rows in L
            for (int c = 0; c < nDets; c++) {
                int b1 = bm_get2(L, l_stride, rank, c);
                int b2 = bm_get2(L, l_stride, best_r, c);
                if (b1 != b2) {
                    L[rank * l_stride + (c >> 3)] ^= (1 << (c & 7));
                    L[best_r * l_stride + (c >> 3)] ^= (1 << (c & 7));
                }
            }
            // Swap y bits at rank and best_r
            swap_bits(y, rank, best_r);

            // Correct swap of rows_p at positions rank and best_r
            int tmp = rows_p[rank];
            rows_p[rank] = rows_p[best_r];
            rows_p[best_r] = tmp;
            inv_rows[rows_p[rank]] = rank;
            inv_rows[rows_p[best_r]] = best_r;
        }

        // L.insert_entry(rank, rank): toggle L[rank][rank]
        L[rank * l_stride + (rank >> 3)] ^= (1 << (rank & 7));

        // Collect rows > rank with 1 in this column and eliminate
        for (int r = rank + 1; r < nDets; r++) {
            if (bm_get2(U, u_stride, r, col)) {
                // U[r] ^= U[rank]
                xor_row_into_row(U, u_stride, r, rank, nMechs);
                // L[r][rank] ^= 1
                L[r * l_stride + (rank >> 3)] ^= (1 << (rank & 7));
                // y[r] ^= y[rank]
                xor_bit(y, r, rank);
            }
        }
        ...
        // Early termination: check if all unpivoted rows (physical pos >= rank) have y=0
        int in_image = 1;
        for (int ii = rank; ii < nDets; ii++) {
            if ((y[ii >> 3] >> (ii & 7)) & 1) { in_image = 0; break; }
        }
        if (in_image) break;
    }

    // Reorder columns: pivots first, non-pivots after
    int *new_cols = malloc(nMechs * sizeof(int));
    if (!new_cols) { free(order); free(U); free(L); free(y); free(rows_p); free(inv_rows); free(pivots); return -1; }
    int pc = 0;
    for (int i = 0; i < nMechs; i++)
        if (pivots[i]) new_cols[pc++] = order[i];
    for (int i = 0; i < nMechs; i++)
        if (!pivots[i]) new_cols[pc++] = order[i];

    // Forward substitution: Lb = P*y_original, where (P*y)[i] = y_original[rows_p[i]]
    uint8_t *b = calloc(rank, 1);
    if (!b) { free(order); free(U); free(L); free(y); free(rows_p); free(inv_rows); free(pivots); free(new_cols); return -1; }
    for (int ri = 0; ri < rank; ri++) {
        int s = 0;
        for (int k = 0; k < ri; k++)
            if (bm_get2(L, l_stride, ri, k) && b[k])
                s ^= 1;
        int orig_row = rows_p[ri];
        int yi = (syn_packed[orig_row >> 3] >> (orig_row & 7)) & 1;
        b[ri] = s ^ yi;
    }

    // Back substitution: UQx = b
    for (int ri = rank - 1; ri >= 0; ri--) {
        int col_j = new_cols[ri];
        int s = 0;
        for (int c = 0; c < nMechs; c++)
            if (bm_get2(U, u_stride, ri, c) && (out_mechs[c >> 3] & (1 << (c & 7))))
                s ^= 1;
        int val = s ^ b[ri];
        if (val)
            out_mechs[col_j >> 3] |= (1 << (col_j & 7));
    }

    free(order); free(U); free(L); free(y);
    free(rows_p); free(inv_rows); free(pivots);
    free(new_cols); free(b);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    for(int i=1;i<argc;i++) {
        if(!strcmp(argv[i],"--bench")) bench=1;
        ...
        else if(!strcmp(argv[i],"--decode-structured")) {
            // OSD-0 on a prebuilt DEM matrix from stdin.
            // Input: [nDets int32] [nMechs int32] [nData int32]
            //        [H: nDets × ((nMechs+7)/8) bytes]
            //        [L: nData × ((nMechs+7)/8) bytes]
            //        [costs: nMechs doubles]
            //        [syndrome: (nDets+7)/8 bytes]
            // Output: [nData bytes, 0/1 per data qubit]
            int nDets, nMechs, nData;
            if (fread(&nDets, 4, 1, stdin) != 1 || nDets < 1 || nDets > MAX_DET) { fprintf(stderr,"bad nDets\n"); return 1; }
            if (fread(&nMechs, 4, 1, stdin) != 1 || nMechs < 1 || nMechs > MAX_MECH) { fprintf(stderr,"bad nMechs\n"); return 1; }
            if (fread(&nData, 4, 1, stdin) != 1 || nData < 1) { fprintf(stderr,"bad nData\n"); return 1; }

            int det_stride = (nDets + 7) / 8;
            int mech_stride = (nMechs + 7) / 8;

            // Read H matrix
            BitMatrix H;
            H.rows = nDets; H.cols = nMechs; H.stride = mech_stride;
            size_t hsz = (size_t)nDets * mech_stride;
            H.data = malloc(hsz);
            if (!H.data) return 1;
            if (fread(H.data, 1, hsz, stdin) != hsz) { free(H.data); fprintf(stderr,"short H\n"); return 1; }

            // Read L matrix
            BitMatrix L;
            L.rows = nData; L.cols = nMechs; L.stride = mech_stride;
            size_t lsz = (size_t)nData * mech_stride;
            L.data = malloc(lsz);
            if (!L.data) { free(H.data); return 1; }
            if (fread(L.data, 1, lsz, stdin) != lsz) { free(H.data); free(L.data); fprintf(stderr,"short L\n"); return 1; }

            // Read costs
            double *costs = malloc(nMechs * sizeof(double));
            if (!costs) { free(H.data); free(L.data); return 1; }
            if (fread(costs, sizeof(double), nMechs, stdin) != (size_t)nMechs) { free(H.data); free(L.data); free(costs); fprintf(stderr,"short costs\n"); return 1; }

            // Read syndrome
            uint8_t *syn_packed = malloc(det_stride);
            if (!syn_packed) { free(H.data); free(L.data); free(costs); return 1; }
            if (fread(syn_packed, 1, det_stride, stdin) != (size_t)det_stride) { free(H.data); free(L.data); free(costs); free(syn_packed); fprintf(stderr,"short syn\n"); return 1; }

            // Run OSD-0
            uint8_t *mech_corr = calloc(mech_stride, 1);
            if (!mech_corr) { free(H.data); free(L.data); free(costs); free(syn_packed); return 1; }

            int ok = osd_decode(&H, costs, syn_packed, mech_corr);

            // Map mechanism correction to data qubit correction via L
            uint8_t *data_corr = calloc(nData, 1);
            if (!data_corr) { free(H.data); free(L.data); free(costs); free(syn_packed); free(mech_corr); return 1; }
            for (int j = 0; j < nMechs; j++) {
                if (mech_corr[j >> 3] & (1 << (j & 7))) {
            ...
            fwrite(data_corr, 1, nData, stdout);
            fflush(stdout);

            free(H.data); free(L.data); free(costs);
            free(syn_packed); free(mech_corr); free(data_corr);
            return 0;
        }
