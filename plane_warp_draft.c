...
static void preprocess_syndrome(int r, int s, uint8_t *syn) {
    // Pass 1: correlated-event detection.
    // DEPOLARIZE2 Z⊗X at anc=(a,b) + data qubit leaves a 2×2 check block
    // with exactly 3 corners toggled (the ancilla corner cancels).
    // Scan all positions at stride 1 — each 2×2 block appears once.
    for(int a=0; a<r; a++) for(int b=0; b<s; b++) {
        ...
        int sum=v00+v10+v01+v11;
        if(sum==3) {
            // Add the missing corner — completes the data-error 4-bit pattern
            if(!v00) syn[a*s+b]^=1;
    ...
    // Pass 2: remaining odd row/col parities (isolated measurement flips,
    // or leftover from multi-correlated events). Original product-code fix.
    int hr=r/2, hs=s/2;
